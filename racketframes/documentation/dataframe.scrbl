...
is a one-dimensional labelled array serving as the primary building block for a DataFrame, making up its rows and columns.
The columns and rows are efficiently accessed using the Indexes generated fron the given data.
...
the operations. Although Indexes have some generation cost, their benefits often make them a good choice to have. Series have one Index and
DataFrames index both columns and rows.
