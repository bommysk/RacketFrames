...
It provides the signifcant benefit of near constant time read speed. Also it allows repeat Index values.
@verbatim{
...
@bold{Index}
An index is an additional structure that is derived from the primary data and a sequence of hashable keys to provide
constant time read access. The index is generated by reading the given data array and hashable keys in parallel and
...
0..n automatically. This efficient mapping in turn results in fast index operations, repeat indexed value flexibility and
...
@bold{DataFrames}
DataFrames are a table like structure which consist of one or more labeled series of equal length.
The representation of a labeled Series is a Column
@codeblock|{ (define-type Column (Pair Label Series)) }|
and constructing a list of Column gives us the main building
blocks of a DataFrame.
@codeblock|{ (define-type Columns (Listof Column)) }|
It also has named attributes labeling the rows providing efficient 2-D read operations through use of
indexes. It allows aggregation and other functions on rows and columns. The ability to clean and filter
data to derive insights. Load data from delimited files and SQLLite Databases with the capability to
scale to millions of rows.

Given a list of Columns we can construct the DataFrame struct.
...
or we can use the friendlier hashtable representation: @codeblock|{ (Sequenceof Label (Sequenceof Any)) }|.
