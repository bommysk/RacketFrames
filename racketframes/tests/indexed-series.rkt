...
(idx->key (build-index-from-list (list 'a 'b 'c 'c 'c 'd)) 3)
...
(define g-series-integer-not-labeled (new-GenSeries (vector 1 2 3 4)))
...
(define g-series-integer (new-GenSeries (vector 1 2 3 4) #:index (build-index-from-list (list 'a 'b 'c 'd))))

; expect exception
;(new-GenSeries (vector 1 2 3 4 5) (build-index-from-list (list 'a 'b 'c 'd 'a)))

(define g-series-integer-2 (new-GenSeries (vector 1 2 3 4 5) #:index (build-index-from-list (list 'a 'b 'c 'd 'e))))

; create float series
(define g-series-float (new-GenSeries (vector 1.5 2.5 3.5 4.5 5.5) #:index (build-index-from-list (list 'a 'b 'c 'd 'e))))

; create symbol series
(define g-series-symbol (new-GenSeries (vector 'e 'f 'g 'h) #:index  (list 'a 'b 'c 'd)))

; point struct
(struct point ([x : Integer] [y : Integer]) #:transparent)

; create point struct series
(define g-series-point (new-GenSeries (vector (point 1 2) (point 3 4) (point 5 6) (point 7 8) (point 9 10)) #:index (list 'a 'b 'c 'd 'e)))
...
(define g-series-struct (new-GenSeries (vector (list #f) (list #f)) #:index (list 'a 'b)))
