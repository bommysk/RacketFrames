...
require([
    ...
    var CustomCellRenderer = TableView.BaseCellRenderer.extend({
        ...
        initialize: function() {
          ...
          for (let token in default_token_model.attributes) {
            ...
            if (! (token in this.isJSEvalColRegexToken)) {
              let isJSEvalColRegexToken = /^js_eval_col\.\/(.*)\/$/
        ...
        render: function($td, cell) {
            ...
            var style_token_values = [];
            // get the value of the token
            var style_token_value = this.tokens.get("style." + this.dashboard_id + "." + cell.field);
            ...
            for (var token in this.tokens.attributes) {
                var coreRegex = this.isStyleRegexToken[token];
                if (coreRegex) {
            ...
            for (var style_token_value of style_token_values) {
              try {
                  // execute the eval to determine the classes names

                  var style_value = eval(style_token_value);
                  // apply CSS classes onto the cell
    ...
    function run(all_view_instances) {
      ...
      $(document).on("full_render_complete", function(e, opts) {
        ...
        all_views.forEach(function(view) {
          ...
          if (mail_format && eval(mail_format)) {              
          ...
          if (datatables_global_token_value && eval(datatables_global_token_value)) {
            if (datatables_id_token_value && (! eval(datatables_id_token_value))) {
              ...
              if (! $.fn.DataTable.isDataTable( table_selector )) {
                ...
                console.log(`${table_selector} setup`);
              } 
            }
          }
        });
