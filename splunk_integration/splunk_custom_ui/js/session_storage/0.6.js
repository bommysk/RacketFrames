require([
  ...
  persist_tokens.forEach(function (persist_token) {
    ...
    if (get_token_value.length > 0) {
      ...
      // not there non-form token counter parts which may have prefixes
      // and suffixes
      if (persist_token.startsWith("form.")) {
        ...
        save_storage(`${app}_${no_form_persist_token}`, no_form_token_value, ttl);
      }
  ...
  persist_tokens_dashboard.forEach(function (persist_token) {
    ...
    if (get_token_value.length > 0) {
      ...
      if (persist_token.startsWith("form.")) {
        let no_form_persist_token = persist_token.slice("form.".length);    
        let no_form_token_value = default_token_model.get(no_form_persist_token);        
        save_storage(`${dashboard_name}_${no_form_persist_token}`, no_form_token_value, ttl);
      }
