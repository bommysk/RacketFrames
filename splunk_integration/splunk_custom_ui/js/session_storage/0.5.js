require([
  ...
  let get_override_tokens = default_token_model.get("get_override_tokens");
  let get_override_tokens_dashboard = default_token_model.get("get_override_tokens_dashboard");
  ...
  if (! get_override_tokens) {
    get_override_tokens = ["form.project"];
  }

  if (! get_override_tokens_dashboard) {
    get_override_tokens_dashboard = [];
  }
  ...
  persist_tokens.forEach(function (persist_token) {
    ...
    if (get_token_value.length > 0) {
      ...
      // we only want get overriding non form token values
      // when dropdown tokens don't have prefixes or suffixes,
      // otherwise those additions would be lost
      // the user must specify tokens that should follow this behavior
      // in $get_override_tokens$
      if (persist_token.startsWith("form.") && get_override_tokens.includes(persist_token)) {
  ...
  persist_tokens_dashboard.forEach(function (persist_token) {
    ...
    if (get_token_value.length > 0) {
      ...
      if (persist_token.startsWith("form.") && get_override_tokens_dashboard.includes(persist_token)) {
        let no_form_persist_token = persist_token.slice("form.".length);
        set_token(no_form_persist_token, get_token_value);
