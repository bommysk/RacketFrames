require([
  ...
  let persist_tokens_arrays = [persist_tokens, persist_tokens_dashboard];
  persist_tokens_arrays.forEach(function (persist_tokens_array) {
    let prefix = (persist_tokens_array === persist_tokens) ? app : dashboard_name;
    persist_tokens_array.forEach(function (persist_token) {
      let get_token_value = url_params.getAll(persist_token);
      let session_token_value = get_storage(`${prefix}_${persist_token}`);
      let ttl = 60;
      ...
      if (get_token_value.length > 0) {
        ...
        // we only want to override form tokens from the url parameters
        // not their non-form token counter parts which may have prefixes
        // and suffixes
        if (persist_token.startsWith("form.")) {
          let no_form_persist_token = persist_token.slice("form.".length);    
          let no_form_token_value = default_token_model.get(no_form_persist_token);        
          save_storage(`${prefix}_${no_form_persist_token}`, no_form_token_value, ttl);
        }

        save_storage(`${prefix}_${persist_token}`, get_token_value, ttl);
      }
      ...
      // lastly default token value has preference
        ...
        if (token_value) {
          save_storage(`${prefix}_${persist_token}`, token_value, ttl);
        }
      }
      ...
      default_token_model.on(`change:${persist_token}`, function () {
        ...
        if (!persist_tokens.includes(`form.${persist_token}`) && !persist_token.startsWith("form.")) {
          save_storage(`${prefix}_form.${persist_token}`, token_value, ttl);
        }
      });
    });
  });
