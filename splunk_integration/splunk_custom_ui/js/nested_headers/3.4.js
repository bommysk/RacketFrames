...
var hideFormatColumns = function (container)  {
     var elements = [];
     $('th', container).each(function(i, el) {
...
var timedCount = function (subview,container) {
  format_headers(subview, container);
  format_counter = format_counter + 1;
}

var is_formatted = function (subview,container) {
...
var format_headers = function (subview,container) {
  
  try {
    if (!is_formatted(subview, container)) {
	  
      var view_id = $(subview).attr("id");
      var table_headers = $("#" + view_id + " table thead tr th");
console.log("START",table_headers);
      table_headers.each(function (i, th)) {
         console.log("iterating " , th);
         if(/^.*\.format\s*$/.test($(th).text())) {
             console.log("removing " , $(th).text());
             $(th).remove();
         } 
      }
      table_headers = $("#" + view_id + " table thead tr th");
    ...
    //hideFormatColumns(container);
  } catch (err) {
    console.log("ERR", err.message);
  }

  if (format_counter < 5)
    setTimeout(function () { timedCount(subview,container) }, 1000);
...
console.log("START");
        subview.on("rendered", function () {
          format_counter = 0;
          timedCount(subview, this.container);
